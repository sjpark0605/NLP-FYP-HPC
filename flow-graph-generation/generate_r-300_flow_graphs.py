...
def obtain_predicted_ner_tags(sentences, keys_list):
  ...
  for sentence, keys in zip(sentences, keys_list):
    input = ner_tokenizer.encode_plus(sentence, truncation=True, is_split_into_words=True, max_length=128, return_tensors='pt').to(device)
...
def construct_phrase_dict(word_dict, ner_dict):
  ...
  for word_key in word_keys:
    if previous_key != None and ner_dict[previous_key].replace("-B", "-I") == ner_dict[word_key]:
...
def obtain_predicted_flow_edges(word_dict, ner_dict, phrase_dict, entity_pairs):
  ...
  for entity_pair in entity_pairs:
    ...
    if label != 'non-edge':
      edges.append((entity_pair, (phrase_dict[entity_pair[0]], phrase_dict[entity_pair[1]]), (ner_dict[entity_pair[0]], ner_dict[entity_pair[1]]), label))
...
def food_recipe_to_true_flow_graph(ner_lines, flow_lines):
  ...
  edges = obtain_true_flow_edges(word_dict, ner_dict, phrase_dict, entity_pairs, labels)
  nodes = set()
...
for ner_file, flow_file in zip(NER_FILES, FLOW_FILES):
  ...
  dest_folder = PROJECT_DIR + 'outputs/generated_flow_graphs/' + filename_without_extension + '/'
