# GPU job script example.
#$ -S /bin/bash
#$ -l h_rt=24:00:0
#$ -l tmem=16G
#$ -wd /cluster/project2/COMP0029_17022125
#$ -j y
#$ -l gpu=true,gpu_type=(titanxp|titanx)
#$ -pe gpu 1
#$ -R y
#$ -o /cluster/project2/COMP0029_17022125/logs/weighted
#$ -m beas


target=$arg1
undersample=$arg2
epochs=$arg3

source /share/apps/source_files/python/python-3.9.5.source
source /share/apps/source_files/cuda/cuda-11.0.source
source /cluster/project2/COMP0029_17022125/.venv/bin/activate
python3 /cluster/project2/COMP0029_17022125/typed-entity-marker/typed_entity_marker_flow_graph_data_processing.py --t "$target" --us "$undersample"
python3 /cluster/project2/COMP0029_17022125/typed-entity-marker/typed_entity_marker_flow_graph_training_loop.py  --t "$target" --us "$undersample" --epochs "$epochs" --weighted
hostname
date
/home/sejipark/test